<?php
include('connect.php');
$valueselected = $_GET['Labvalue']; //'D-DIMER';// 
...
$Item_desc_name=mysqli_query($con,"select * from test_item_cat where item_name='$valueselected' and Tst_Cat_ID='TCAT105'");
while($Item_desc_row = mysqli_fetch_array($Item_desc_name)){
    $valueselected=$Item_desc_row['Item_desc'];
...
$result=mysqli_query($con,"select distinct pr.Person_ID, p.Patient_ID,  tc.Test_Category, tic.item_name, pe.result,pe.date,pe.time, tac.Min_Range, tac.Max_Range, tac.Exact_Range, tac.units from patient_exam pe join patient_visit pv on pe.Visit_ID = pv.Visit_ID
                            ...
                            where p.Patient_ID = '$patient_id' and tc.Tst_Cat_ID = 'TCAT105' and pe.date BETWEEN  '$formatted_start_date' and '$formatted_end_date' and tic.Item_desc='$valueselected'");
