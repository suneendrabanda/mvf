...
$result=mysqli_query($con,"select  t.tst_cat_ID,tc.item_name,tc.Item_desc from test_item_cat tc join test_cat t on tc.Tst_cat_ID=t.Tst_cat_id where t.Tst_cat_ID='TCAT103' order by tc.item_name");
while($row = mysqli_fetch_array($result)){
     $item_desc=$row['Item_desc'];
     //echo $item_desc.'<br>';
     $Range_results=mysqli_query($con,"SELECT * FROM `test_range_age_category` where `AgeCat_ID`='$age_cat' and `Tst_Cat_ID`='TCAT103' and Item_desc='$item_desc'");
