...
$arr = array();
 if($shiftselected=='day'){
    ...
    elseif($shiftselected=='evening'){
        ...
        $result=mysqli_query($con,"select SUBSTR(X.result, 1, LOCATE('/', X.result)-1)as vital , SUBSTR(X.result, LOCATE('/', X.result)+1, char_length(X.result)- LOCATE('/', X.result)) as deno , X.time from (select pr.fname, pr.lname, vs.Name, ve.date, ve.time, ve.result from vital_signs vs join VS_Exam ve on vs.VS_ID = ve.VS_ID join Patient_Visit pv on pv.Visit_ID = ve.Visit_ID join patient p on p.patient_ID = pv.patient_ID join person pr on pr.person_ID = p.person_ID) X where X.fname = 'Sandra' and X.time in ('2300','2400','0100','0200','0300','0400','0500','0600') and X.name = 'BP'  and X.date between '$formatted_start_date' and '$formatted_end_date' order by X.time");
