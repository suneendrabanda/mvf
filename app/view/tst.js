...
//            },
    ...
    OnIOPageload:function(){
        ...
        var OuputLastUpdatedDate=StartDate;
        var shift;
        var time;
        var TableValues;
       ...
       outputStore.load({
                    ...
                    //var OPEndDate=Ext.Date.format(Ext.Date.add(new Date(OuputLastUpdatedDate),Ext.Date.DAY,7),'m/d/Y');
                     ...
                     OutputTableStore.load({
                            ...
                            callback:function(records){
                                ...
                                var No_of_Results_Fetch=TableStore.getCount();
                                var Total_Output=0;
                                TableValues='<table><thead><tr style="border-bottom:1px solid #a5a399"><th style="padding: 0 82px 8px 0">Date</th><th style="padding: 0 66px 8px 0">Time</th><th style="padding: 0 110px 8px 0">Output</th><th style="padding: 0 80px 8px 0">Result</th></tr></thead><tbody>';
                                //var tablepanel=this.getOutputResultsPanel();
                                for(var i=0;i<No_of_Results_Fetch;i++){
                                    TableValues+='<tr style="padding: 10px 0 0 0"><td>'+records[i].data.date+'</td><td>'+records[i].data.time+'</td><td>'+records[i].data.Name+'</td><td>'+records[i].data.result+'</td></tr>';
                                    Total_Output+=parseInt(records[i].data.result);
        ...
        // print table after some time
        Ext.defer(function() {
                var tablepanel=this.getOutputResultsPanel();          
                tablepanel.setHtml(TableValues);
            }, 1000, this);
        //load Intake chart store
        // get last updated intake date
                  ...
                  callback:function(records,success){
                      ...
                      if(no_of_records>0){
                        ...
                        if(parseInt(time)>700&& parseInt(time)<1500){
                         ...
                         //console.log(IntakeLastUpdateDate);
                        var IntakeChartStore=Ext.getStore('IOPageIntakeChartStore');
                        var EndDate=Ext.Date.format(Ext.Date.add(new Date(IntakeLastUpdateDate),Ext.Date.DAY,7),'m/d/Y');
                        IntakeChartStore.load({
                            params:{
                                        shiftvalue: shift,
                        ...
                        IntakeTableStore.load({
                            ...
                            callback:function(records){
                                ...
                                var No_of_Results_Fetch=TableStore.getCount();
                                var Total_intake=0;
                                 IntakeTableValues='<table><thead><tr style="border-bottom:1px solid #a5a399"><th style="padding: 0 82px 8px 0">Date</th><th style="padding: 0 66px 8px 0">Time</th><th style="padding: 0 110px 8px 0">Intake</th><th style="padding: 0 80px 8px 0">Result</th></tr></thead><tbody>';
                                //var tablepanel=this.getIntakeResultsPanel();
                                for(var i=0;i<No_of_Results_Fetch;i++){
                                    IntakeTableValues+='<tr style="padding: 10px 0 0 0"><td>'+records[i].data.date+'</td><td>'+records[i].data.time+'</td><td>'+records[i].data.Name+'</td><td>'+records[i].data.result+'</td></tr>';
                                    Total_intake+=parseInt(records[i].data.result);
                                }
                                IntakeTableValues+='<tr style="padding: 10px 0 0 0;border-top:1px solid #a5a399"><td>'+''+'</td><td>'+''+'</td><td style="padding-top:14px">'+'Total Intake'+'</td><td style="padding-top:14px">'+Total_intake+'</td></tr>';
                                IntakeTableValues+='</tbody></table>';
                            }
                        });
                  }
                  }
